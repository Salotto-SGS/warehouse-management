    ...
    if (isset($type) && $type == "missing_product") {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            if ($row['name'] == 'consegnato') {
                $stmt = $db->prepare("SELECT deliveryDate FROM `order` WHERE code=:userCode");
                $stmt->bindParam(':userCode', $code, PDO::PARAM_STR, 8);
                $stmt->execute();
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $date = new DateTime();
                    $deliveryDate = new DateTime($row['deliveryDate']);
                    $interval = $date->diff($orderDate);
                    $elapsed = $interval->format('%a');
                    if ($elapsed > 14) {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            if ($row['name'] == 'consegnato') {
                $stmt = $db->prepare("SELECT deliveryDate FROM `order` WHERE code=:user_code");
                $stmt->bindParam(':user_code', $code, PDO::PARAM_STR, 8);
                $stmt->execute();
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $date = new DateTime();
                    $deliveryDate = new DateTime($row['deliveryDate']);
                    $interval = $date->diff($deliveryDate);
                    $elapsed = $interval->format('%a');
                    if ($elapsed > 5) {
